// /dashboard/director/teachers/page.jsx
"use client";
...
export default function DirectorTeachers() {
  ...
  const sendWelcomeEmail = async (teacherEmail, teacherName) => {
    // Cette fonction enverrait un email de bienvenue en pratique
    // Pour l'instant, on simule l'envoi
    alert(`Email de bienvenue envoyé à ${teacherName} (${teacherEmail})`);
  };
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              {school?.name || "École"} - {teachers.length} enseignant(s) inscrit(s)
            </p>
          </div>
          <Link
              ...
              {filteredTeachers.length > 0 ? (
                  ...
                  <tr key={teacher.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                        {teacher.subject || "Non spécifié"}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{teacher.profiles.email}</div>
                      <div className="text-sm text-gray-500">{teacher.profiles.phone || "N/A"}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        teacher.profiles.status === 'active' 
                          ...
                          : teacher.profiles.status === 'pending'
                          ? 'bg-yellow-100 text-yellow-800'
                          : 'bg-red-100 text-red-800'
                      }`}>
                        {teacher.profiles.status === 'active' ? 'Actif' : 
                         teacher.profiles.status === 'pending' ? 'En attente' : 'Rejeté'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex justify-end space-x-2">
                        <button
                          onClick={() => sendWelcomeEmail(teacher.profiles.email, `${teacher.profiles.first_name} ${teacher.profiles.last_name}`)}
                        ...
                        <Link
                          href={`/dashboard/director/teachers/${teacher.id}`}
                        ...
                        <Link
                          href={`/dashboard/director/teachers/${teacher.id}/edit`}
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-24 text-center">
                    <FontAwesomeIcon icon={faChalkboardTeacher} className="mx-auto h-12 w-12 text-gray-400" />
                    <h3 className="mt-2 text-sm font-medium text-gray-900">Aucun enseignant trouvé</h3>
                    <p className="mt-1 text-sm text-gray-500">
                      {searchTerm || subjectFilter ? "Aucun enseignant ne correspond aux critères de recherche." : "Commencez par ajouter votre premier enseignant."}
                    </p>
            ...
            <div className="flex-1 flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{filteredTeachers.length}</span> enseignants
                </p>
              </div>
          ...
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              <div className="flex-shrink-0 bg-blue-100 rounded-md p-3">
                <FontAwesomeIcon icon={faChalkboardTeacher} className="h-6 w-6 text-blue-600" />
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Enseignants actifs</dt>
                  <dd className="text-lg font-medium text-gray-900">
                    {teachers.filter(t => t.profiles.status === 'active').length}
                  </dd>
                </dl>
              </div>
            </div>
          ...
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              <div className="flex-shrink-0 bg-yellow-100 rounded-md p-3">
                <FontAwesomeIcon icon={faChalkboardTeacher} className="h-6 w-6 text-yellow-600" />
              </div>
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">En attente</dt>
                  <dd className="text-lg font-medium text-gray-900">
                    {teachers.filter(t => t.profiles.status === 'pending').length}
                  </dd>
                </dl>
              </div>
            </div>
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <div className="px-4 py-5 sm:p-6">
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  <dt className="text-sm font-medium text-gray-500 truncate">Matières différentes</dt>
                  <dd className="text-lg font-medium text-gray-900">
                    {subjects.length}
                  </dd>
                </dl>
